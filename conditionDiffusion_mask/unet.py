...
class AttnBlock(nn.Module):
    def __init__(self, in_ch: int):
        ...
        self.proj_q = nn.Conv2d(
            in_ch, in_ch, kernel_size=1, stride=16, padding=0)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, C, H_, W_ = q.shape  # 새로운 크기를 가져옴
        q = q.permute(0, 2, 3, 1).view(B, H_ * W_, C)
        k = k.view(B, C, H_ * W_)
        w = torch.bmm(q, k) * (int(C) ** (-0.5))
        assert list(w.shape) == [B, H_ * W_, H_ * W_]
        w = F.softmax(w, dim=-1)

        v = v.permute(0, 2, 3, 1).view(B, H_ * W_, C)
        h = torch.bmm(w, v)
        assert list(h.shape) == [B, H_ * W_, C]
        h = h.view(B, H_, W_, C).permute(0, 3, 1, 2)
        h = self.proj(h)
        h = F.interpolate(h, size=(H, W), mode='bilinear',
                          align_corners=False)  # 원래 크기로 업샘플링
...
class Unet(nn.Module):
    def __init__(self, in_ch=3, mod_ch=64, out_ch=3, ch_mul=[1, 2, 4, 8], num_res_blocks=2, cdim=10, use_conv=True, droprate=0, dtype=torch.float32):
        ...
        self.cemb_layer1 = nn.Sequential(
            nn.Linear(self.cdim, tdim),
            nn.SiLU(),
            nn.Linear(tdim, tdim),
        )
        ...
        for i, mul in enumerate(self.ch_mul):
            ...
            if i != len(self.ch_mul) - 1:
                self.downblocks.append(EmbedSequential(
                    Downsample(now_ch, now_ch, self.use_conv)))
                chs.append(now_ch)
        ...
        for i, mul in list(enumerate(self.ch_mul))[::-1]:
            ...
            for j in range(num_res_blocks + 1):
                layers = [
                    ResBlock(now_ch+chs.pop(), nxt_ch,
                             tdim, tdim, self.droprate),
                    AttnBlock(nxt_ch)
                ]
                now_ch = nxt_ch
...
class UnetWithMask(nn.Module):
    def __init__(self, in_ch=3, mask_ch=1, mod_ch=64, out_ch=3, ch_mul=[1, 2, 4, 8], num_res_blocks=2, cdim=10, use_conv=True, droprate=0, dtype=torch.float32):
        ...
        # Define timestep and condition embeddings
        tdim = mod_ch * 4
        self.temb_layer = nn.Sequential(
            nn.Linear(mod_ch, tdim),
            nn.SiLU(),
        ...
        # Downsampling layers
        self.downblocks = nn.ModuleList([
            EmbedSequential(nn.Conv2d(self.in_ch, self.mod_ch, 3, padding=1))
        ])
        now_ch = self.ch_mul[0] * self.mod_ch
        chs = [now_ch]
        for i, mul in enumerate(self.ch_mul):
